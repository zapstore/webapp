/**
 * Profile mention search.
 *
 * Priority for @mention suggestions:
 *   1. Thread participants
 *   2. Logged-in user's contacts/follows from Dexie
 *   3. Vertexlab NIP-50 search — relay-side text matching, appended after local results
 */
import { writable } from 'svelte/store';
import { nip19 } from 'nostr-tools';
import { SITE_ICON, VERTEXLAB_RELAY, ZAPSTORE_RELAY } from '$lib/config';
import { parseProfile } from '$lib/nostr/models';
import { queryEvent, queryEvents } from '../storage/dexie.js';
import { fetchFromRelays, fetchProfilesBatch } from './service.js';
	...
	void (async () => {
		try {
			...
			if ((!kind3 || kind3.length === 0) && typeof window !== 'undefined') {
				kind3 = await fetchFromRelays(
					[ZAPSTORE_RELAY],
					{ kinds: [KIND_CONTACT_LIST], authors: [userPubkey], limit: 1 },
					{ timeout: 5000, feature: 'profile-search' }
				);
			}
