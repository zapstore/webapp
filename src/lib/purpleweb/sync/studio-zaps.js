/**
 * Studio zap totals per app per day from kind 9735 on Zapstore.
...
import { queryEvents, putEvents } from '../storage/dexie.js';
import { fetchFromRelays, parseZapReceipt } from './service.js';

const STUDIO_ZAP_RELAYS = [ZAPSTORE_RELAY];
/** Studio loads 2x the selected window for prior-period percent tickers; cap must cover longer ranges. */
...
export async function loadZapAppData(pubkeyHex, apps, days) {
	...
	if (aTags.length > 0) {
		merge(
			await fetchFromRelays(
				STUDIO_ZAP_RELAYS,
				{ kinds: [EVENT_KINDS.ZAP_RECEIPT], '#a': aTags, since, limit: ZAP_FETCH_LIMIT },
	...
	if (eventIds.length > 0) {
		...
		const fE = { kinds: [EVENT_KINDS.ZAP_RECEIPT], '#e': eventIds, since, limit: ZAP_FETCH_LIMIT };
		const fEu = { kinds: [EVENT_KINDS.ZAP_RECEIPT], '#E': eventIds, since, limit: ZAP_FETCH_LIMIT };
