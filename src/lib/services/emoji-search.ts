...
export const DEFAULT_EMOJIS: EmojiData[] = UNICODE_EMOJIS.slice(0, 24).map((e) => ({
 ...
 */
function getEmojiTags(event: NostrEvent): EmojiTag[] {
 ...
 */
export function createEmojiSearch(userPubkey: string | null = null) {
	// Map of shortcode -> emoji data
	const emojis = new Map<string, EmojiData>();
	 ...
	 */
	function addEmoji(shortcode: string, url: string, source = 'custom'): void {
	 ...
	 */
	function addUserEmojiList(event: NostrEvent): void {
	 ...
	 */
	function addEmojiSet(event: NostrEvent): void {
	 ...
	 */
	async function search(query: string): Promise<EmojiData[]> {
		// Ensure initialized
		await init();
		...
		if (!query || query.length < 1) {
			...
			const userEmojis: EmojiData[] = [];
			const customEmojis: EmojiData[] = [];
			const unicodeEmojis: EmojiData[] = [];
...
const emojiSearchCache = new Map<string, ReturnType<typeof createEmojiSearch>>();

/**
 * Gets or creates an emoji search service for a user
 */
export function getEmojiSearch(userPubkey: string | null = null) {
 ...
 */
export function clearEmojiSearchCache(): void {
 ...
 */
export function createSearchEmojisFunction(userPubkey: string | null = null) {
 ...
 */
export async function searchEmojis(query: string): Promise<EmojiData[]> {
