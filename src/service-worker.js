...
sw.addEventListener('fetch', (event) => {
    ...
    const isPrecached = PRECACHE_ASSETS.includes(urlString) || PRECACHE_ASSETS.includes(url.pathname);
    if (isPrecached) {
    ...
    event.respondWith(fetch(event.request).catch(() => caches.match(event.request).then((cached) => cached ||
