<script lang="js">
	...
	const isOwner = $derived(
		getIsSignedIn() && getCurrentPubkey() && stack?.pubkey && getCurrentPubkey() === stack.pubkey
	);
	...
	function getStackAppsClipMaxHeightPx() {
		if (!browser) return STACK_APPS_CLIP_MAX_HEIGHT_DESKTOP_PX;
	...
	$effect(() => {
		...
		const measure = () => {
			...
			stackAppsHeightOverflow = apps.length > STACK_APPS_PREVIEW_MAX || el.scrollHeight > maxH + 2;
		};
		measure();
	...
	const displayDescription = $derived(
		stack
			? stackDisplayDescription(
					{ title: stack.title, description: stack.description },
					displayTitle
				)
			: ''
	);
							...
							{#if stack?.creator}
								...
								>
									<ProfilePic
										...
										size="bubble"
									/>
									<span class="detail-publisher-name"
										>{stack.creator.name || stack.creator.npub?.slice(0, 12) + '...'}</span
									>
								</a>
							{:else}
								<div></div>
							...
							{#if stack?.createdAt}
								<div class="detail-publisher-trailing">
									<Timestamp
										...
										size="xs"
										className="detail-publisher-timestamp"
									/>
									{#if zapTarget}
						...
						<div class="section-container stack-apps-section">
							...
							{:else}
								<div class="placeholder-content">
									<p class="text-muted-foreground regular14">No apps found in this stack.</p>
								</div>
							{/if}
						</div>
...
<style>
	...
	@media (min-width: 768px) {
		...
		.stack-apps-section .stack-apps-grid-item:nth-last-child(2):nth-child(odd):not(:last-child) {
			border-bottom: none;
		}
	}
	...
	.stack-apps-fade {
		...
		background: linear-gradient(
			to bottom,
			transparent 0%,
			color-mix(in srgb, var(--black) 40%, transparent) 35%,
			var(--black) 100%
		);
		pointer-events: none;
		z-index: 2;
